...
fs.ensureDirSync(reports, function (err) {
	if (err) {
		log('The Reports Folder has NOT been created: ' + err.stack);
	}
});
fs.ensureDirSync(fileDnldFldr, function (err) {
	if (err) {
		log('The File Download Folder has NOT been created: ' + err.stack);
	}
});
fs.ensureDir(docsFolder, function (err) {
	if (err) {
		log('The Docs Folder has NOT been created: ' + err.stack);
	}
});
fs.ensureFile(fileName, function (err) {
	if (err) {
		log('The fileName File has NOT been created: ' + err.stack);
	}
});
...
new Promise(async function (resolve, reject) {
	try {
		...
		log('cucumber integration has failed ' + err.message);
		await reject(err);
		throw err;
	}
});
